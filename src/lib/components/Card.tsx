...
const Card: FC<ICard & ICardStyled> = ({
  ...
  const innerClick = useCallback(
    e => {
      if (e.target.tagName !== "CANVAS" && !!onClick && !MenuVisibility) {
        onClick(e);
      }
    },
  ...
  return (
    <div className="card-component-module" onClick={innerClick}>
      <Reset />
      <Container
      ...
      >
        {headerText && (
          <CardHeader media={media}>
            {thumbnail && (
              <Thumbnail appearance={appearance} theme={themeToApply}>
                {thumbnail}
              </Thumbnail>
            )}
            <HeaderTextContainer>
